...
data_test = pd.read_csv('test_df_sample_500.csv')
...
def get_shap_val_local(client_id):
    """Récupère les shap values du client via l'API pour une interprétation locale.
    :param: client_id (int).
    :return: shap_local
    """
...
def df_voisins(id_client):
    ...
    :return: data_voisins
    """
    url_get_df_voisins = API_URL + "similar_clients/" + str(id_client)
...
def boxplot_graph(id_client, feat, df_vois):
    """Affiche les boxplots des variables renseignéees en paramètre pour chaque target.
    Affiche également la position du client dont l'ID est renseigné en paramètre dans ce graphique.
    Affiche les 10 plus proches voisins du client sur les boxplots.
...
# Sidebar
with st.sidebar:
    logo = Image.open('img/pret_a_depenser.png')
    ...
    st.markdown("""---""")
    st.write("Created by Méric Manuel Kucukbas")


if page == "Home":
    st.title("Dashboard Prêt à dépenser - Home Page")
    st.markdown("Ce site contient un dashboard interactif permettant aux chargés"
                "de relation client d'évaluer la probabilité de remboursement d'un crédit et classifie la demande de crédit en conséquence.\n"
...
if page == "Interprétation locale":
    ...
    if locale:
        ...
        # fig, ax = plt.subplots()
        fig = shap.waterfall_plot(shap_val, max_display=nb_features)
        st.pyplot(fig)
        ...
        with st.expander("Explication du graphique", expanded=False):
            st.caption("Ici sont affichées les caractéristiques influençant de manière locale la décision. "
                       "C'est-à-dire que ce sont les caractéristiques qui ont influencé la décision pour ce client "
                       "en particulier.")
...
if page == "Interprétation globale":
    ...
    if globale:
        ...
        with st.expander("Explication du graphique", expanded=False):
            st.caption("Ici sont affichées les caractéristiques influencant de manière globale la décision.")
